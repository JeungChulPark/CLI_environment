...
G2O_USE_OPTIMIZATION_LIBRARY(eigen)  // conda-forge g2o has no cholmod solver; eigen + csparse instead
...
int GraphSLAM::optimize(int num_iterations, int min_edges) {
  g2o::SparseOptimizer* graph = dynamic_cast<g2o::SparseOptimizer*>(this->graph.get());
  if(static_cast<int>(graph->edges().size()) < min_edges) {
    return -1;
  }
  ...
  std::cout << "time: " << boost::format("%.3f") % elapsed.count() << "[sec]" << std::endl;
