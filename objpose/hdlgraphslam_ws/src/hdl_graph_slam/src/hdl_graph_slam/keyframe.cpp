...
void KeyFrame::save(const std::string& directory) {
  ...
  const int64_t stamp_nanoseconds = stamp.nanoseconds();
  ofs << "stamp " << stamp_nanoseconds / 1000000000LL << " " << stamp_nanoseconds % 1000000000LL << "\n";
...
bool KeyFrame::load(const std::string& directory, g2o::HyperGraph* graph) {
  ...
  while(!ifs.eof()) {
    ...
    if(token == "stamp") {
      ...
      uint32_t nsec = 0;
      ifs >> sec >> nsec;
      stamp = rclcpp::Time(sec, nsec, RCL_ROS_TIME);
