...
InformationMatrixCalculator::InformationMatrixCalculator(rclcpp::Node& node) : InformationMatrixCalculator() {
  use_const_inf_matrix = node.declare_parameter<bool>("use_const_inf_matrix", use_const_inf_matrix);
  const_stddev_x = node.declare_parameter<double>("const_stddev_x", const_stddev_x);
  const_stddev_q = node.declare_parameter<double>("const_stddev_q", const_stddev_q);

  var_gain_a = node.declare_parameter<double>("var_gain_a", var_gain_a);
  min_stddev_x = node.declare_parameter<double>("min_stddev_x", min_stddev_x);
  max_stddev_x = node.declare_parameter<double>("max_stddev_x", max_stddev_x);
  min_stddev_q = node.declare_parameter<double>("min_stddev_q", min_stddev_q);
  max_stddev_q = node.declare_parameter<double>("max_stddev_q", max_stddev_q);
  fitness_score_thresh = node.declare_parameter<double>("fitness_score_thresh", fitness_score_thresh);
}
