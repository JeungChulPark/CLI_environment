...
lifecycle 단계는 SLAM 궤적과 리그 extrinsic 이 있어야 돌아간다(객체를 map 프레임에
...
def run_lifecycle(data: Path, out: Path, backend: str, gate: float,
    ...
    cals = sorted((data / "calib").glob("*.npy")) if (data / "calib").is_dir() else []
    ...
    for label, path in (("검출(objects.json)", dets_json), ("SLAM 궤적", traj),
        ...
        if not path.is_file():
            ...
            return None
    if len(cals) != 1:
        C.log(f"lifecycle 건너뜀: {data}/calib 에 4x4 extrinsic .npy 가 정확히 하나 "
              f"있어야 한다 (찾은 것 {len(cals)}개)")
        return None

    with open(info_json, encoding="utf-8") as f:
        color_topic = (json.load(f).get("sam", {}).get("color_topic")
                       or "/camera/camera/color/image_raw")
    X = OL.load_npy_4x4(str(cals[0]))
    ...
    C.log(f"lifecycle({backend}): SLAM 포즈 {len(poses)} · SAM 프레임 {len(timestamps)} · "
          f"검출 {n_det}건/{len(detections)}프레임 · extrinsic {cals[0].name}")
    if cam_K is None:
    ...
    payload["inputs"] = {"detections": str(dets_json), "trajectory": str(traj),
                         "extrinsic": str(cals[0]), "backend": backend}
