...
def main() -> int:
    ...
    xs = sorted((data / "calib").glob("*.npy")) if (data / "calib").is_dir() else []
    if len(xs) != 1:
        C.die(f"{data}/calib/ 에 4x4 extrinsic .npy 가 정확히 하나 있어야 한다 (찾은 것 {len(xs)}개).\n"
              f"  X = T_camSLAM_camSAM 는 두 카메라의 리그 변환이다. 0807 리그 값은\n"
              f"  integration/output/0807_185223_circle8/X_camSLAM_camSAM.npy 에 이미 있다.\n"
              f"  새로 추정하려면 SAM bag 에 ORB3 를 한 번 더 돌려 rig_offset.py 로 hand-eye "
              f"를 풀어야 한다 — 추가 산출물이 생기므로 자동으로 하지 않는다.")
    X = np.load(xs[0])
    C.log(f"extrinsic X = {xs[0]}")
