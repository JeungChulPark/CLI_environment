...
# Paths
# ---------------------------------------------------------------------------
# PERF-PROBE COPY: this file lives in _perf_bottleneck_probe_2026_07_06/copied/,
# two levels below the real sam6d_ws root — resolve REPO_ROOT to the original
# workspace so checkpoints/templates/frames are read from their real locations.
...
def dinov2_forward(model, crop_chw, device, want_patch=False,
    ...
    PERF-PROBE improvement (1-refined): with patch_stays_on_device=True the
    normalized patch tokens are returned as a GPU tensor — the caller keeps
    only the current-best reference and pays ONE device->host transfer for
    the winner instead of one per proposal. Values are identical either way
    (normalization already happens on-device; only the copy moment changes).
    """
...
def dinov2_forward_batch(model, crops, device):
    """PERF-PROBE opt2: ONE forward for a whole frame's crops.

    crops: list of [3,224,224] tensors. Returns (cls[B,384] on CPU,
    patch[B,256,384] kept ON DEVICE — transfer only the winners).
    Same math as dinov2_forward called B times; batching only removes the
...
def segment_boxes(seg, bgr, boxes, device):
    """PERF-PROBE opt2: ONE MobileSAM call for ALL bbox prompts of a frame.

    The expensive image encoding runs once; each bbox only pays the decoder.
    Returns list of full-image boolean masks aligned to `boxes` (None where
    missing). Mask<->box correspondence relies on ultralytics returning masks
    in prompt order — verified empirically against per-box calls before use.
    """
