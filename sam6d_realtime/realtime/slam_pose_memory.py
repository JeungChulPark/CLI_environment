...
class ObjectAnchorManager:
    ...
    DEFAULTS = {
        ...
        "sym_step_deg": 10,
        "symmetry_axes": {},
        # 등록 전 soft 보정: 최근 N개 관측의 medoid 를 원 측정 대신 출력 (0=끔).
        # 2fps 실측(_temporal_correction_probe_2026_08_30)에서 K=5 가 무릎점 —
        # 나쁜표시 23.9%→9.3%. 앵커와 달리 잠기지 않아 flip 에서 스스로 회복한다.
        "soft_window": 5,
    }
    ...
    def __init__(self, map_id, config=None):
        ...
        if int(self.config["soft_window"]) < 0:
            raise ValueError("soft_window must be >= 0 (0 disables soft output)")
    ...
    def soft_map_pose(self, object_id):
        """등록 전 soft 보정 포즈: 최근 soft_window개 map 관측의 dominant-cluster medoid.

        앵커 등록과 같은 임계·같은 medoid 함수를 쓰지만, 앵커처럼 고정되지 않고
        매 호출 최근 관측 다수를 다시 따른다. 관측이 2개 미만이면 None (=원 측정 유지).
        """
        window = int(self.config["soft_window"])
        if window <= 0 or object_id in self.anchors:
            return None
        poses = list(self.histories.get(object_id, ()))[-window:]
        if len(poses) < 2:
            return None
        axis = self._axis(object_id)
        step = int(self.config["sym_step_deg"])
        rotation = float(self.config["register_rotation_deg"])
        translation = float(self.config["register_translation_mm"]) / 1000.0
        cluster = dominant_pose_cluster(poses, rotation, translation, axis, step)
        medoid = pose_medoid(poses, cluster, axis, step, rotation, translation)
        return poses[medoid].copy()

    def soft_camera_pose(self, object_id, camera_to_map):
        pose = self.soft_map_pose(object_id)
        if pose is None or not valid_se3(camera_to_map):
            return None
        return np.linalg.inv(np.asarray(camera_to_map, float)) @ pose
