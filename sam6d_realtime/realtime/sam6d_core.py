...
class Sam6DCore:
    ...
    def _apply_anchors(self, rows, shadows, hits, depth, K, image_shape, slam_context):
        ...
        # 등록 전 soft 보정: 앵커를 기다리는 동안 원 측정 대신 최근 관측 medoid 를
        # 출력한다 (soft_window 관측 다수결이라 단발 flip 이 화면에 나가지 않는다).
        # 원 측정은 raw_R/raw_t_mm 으로 보존한다.
        soft_objects = []
        for row in rows:
            if row["object"] in manager.anchors:
                continue
            pose = manager.soft_camera_pose(row["object"], twc)
            if pose is None:
                continue
            row["raw_R"], row["raw_t_mm"] = row["R"], row["t_mm"]
            row["R"] = [[round(float(v), 6) for v in matrix_row]
                        for matrix_row in pose[:3, :3]]
            row["t_mm"] = [round(float(v) * 1000.0, 4) for v in pose[:3, 3]]
            row["pose_source"] = "soft_medoid"
            row["anchor_state"] = "soft"
            soft_objects.append(row["object"])
