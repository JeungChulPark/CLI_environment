...
아래 띠는 같은 객체의 연속 추정 사이 회전 변화량(Δrot) 이력이다.
    ...
    python realtime/sam6d_viewer.py --headless --http 8090       # 브라우저로 보기
                                    # → http://localhost:8090  (MJPEG + 실시간 상태)
"""
...
<title>SAM-6D Live</title><style>
 body{margin:0;background:#111;color:#ddd;font:14px/1.5 system-ui,sans-serif}
 header{padding:8px 14px;background:#1b1b1b;display:flex;gap:18px;align-items:baseline}
 header b{color:#fff;font-size:16px} header span{color:#9ad}
 #view{width:100%;display:block}
 #dets{padding:8px 14px;display:flex;gap:10px;flex-wrap:wrap}
 .d{background:#1e1e1e;border-radius:6px;padding:6px 10px;border-left:4px solid #888}
 .d b{color:#fff} .d small{color:#aaa;display:block}
 .src-sam6d{border-color:#c53030}.src-soft_medoid{border-color:#2f855a}
 .src-slam_anchor{border-color:#2b6cb0}
 footer{padding:6px 14px;color:#777;font-size:12px}
</style></head><body>
<header><b>SAM-6D Live</b><span id="hz">-</span><span id="ms">-</span>
 <span id="src" style="color:#888">raw=빨강 · soft=초록 · anchor=파랑</span></header>
<img id="view" src="/stream">
<div id="dets"></div>
<footer>LIVE | CAD points | 3D bbox — sam6d_viewer.py --http (MJPEG, 상태 0.5s 폴링)</footer>
<script>
const img = document.getElementById('view');
function reconnect(){ img.src = '/stream?t=' + Date.now(); }
img.addEventListener('error', () => setTimeout(reconnect, 2000));
let statsDown = false;
async function poll(){
 try{
  const s = await (await fetch('/stats')).json();
  if (statsDown){ statsDown = false; reconnect(); }   // 뷰어 재시작 후 스트림 재연결
  document.getElementById('hz').textContent = (s.hz_in||0).toFixed(1)+' Hz in';
  const m = s.ms||{};
  document.getElementById('ms').textContent =
    (m.total||0)+'ms [yolo '+(m.yolo||0)+' / ism '+(m.ism||0)+' / pem '+(m.pem||0)+']';
  document.getElementById('dets').innerHTML = (s.dets||[]).map(d =>
   '<div class="d src-'+(d.pose_source||'sam6d')+'"><b>'+d.object+'</b> '+
   (d.score!=null?d.score.toFixed(2):'')+
   '<small>'+(d.pose_source||'sam6d')+(d.anchor_state?' · '+d.anchor_state:'')+
   (d.drot!=null?' · drot '+d.drot.toFixed(0)+'°':'')+'</small></div>').join('');
 }catch(e){ statsDown = true; }
 setTimeout(poll, 500);
}
poll();
</script></body></html>"""
...
class WebSink:
    """--http: 렌더된 canvas 를 MJPEG 로, 최근 결과를 /stats JSON 으로 서빙."""

    def __init__(self, port, jpeg_fps=12.0, quality=80):
        self.cond = threading.Condition()
        self.jpeg = None
        self.seq = 0
        self.stats = {}
        self.period = 1.0 / jpeg_fps
        self.quality = int(quality)
        self._last_enc = 0.0
        sink = self

        class Handler(BaseHTTPRequestHandler):
            def log_message(self, *args):      # 요청 로그로 콘솔을 덮지 않는다
                pass
            ...
            def do_GET(self):
                if self.path in ("/", "/index.html"):
                    ...
                    body = _json.dumps(sink.stats).encode("utf-8")
                    ...
                    try:
                        while True:
                            with sink.cond:
                                sink.cond.wait_for(lambda: sink.seq != seen, timeout=2)
                                if sink.jpeg is None:
                                    continue
                                buf, seen = sink.jpeg, sink.seq
        ...
        print(f"[view] 브라우저 뷰: http://localhost:{port}")

    def update(self, canvas, stats):
        self.stats = stats
        now = time.time()
        if now - self._last_enc < self.period:
            return
...
def main():
    ...
    ap.add_argument("--http", type=int, default=0,
                    help="포트 지정 시 브라우저 뷰 서빙 (예: 8090 → http://localhost:8090)")
    ap.add_argument("--http-fps", type=float, default=12.0, help="MJPEG 인코딩 fps")
    a = ap.parse_args()
    web = WebSink(a.http, a.http_fps) if a.http else None

    MP, BOX, cidx = load_models()
    print(f"[view] CAD 점군 {len(MP)} 객체")
    fr = jr = None
    waited = 0
    while True:                                # 수신·추론이 먼저 뜰 때까지 기다린다
        try:
            fr = fr or FrameReader()
            jr = jr or JsonReader()
            break
        except FileNotFoundError:
            # 창 모드는 60초 후 포기(예전 동작), 웹 모드는 서버를 띄워 둔 채 무한 대기
            # — 브라우저를 열어 두고 파이프라인을 나중에/다시 띄워도 붙는다.
            waited += 1
            if web is None and waited >= 600:
                raise SystemExit("[view] 공유메모리를 찾지 못했다. 수신/추론 프로세스를 먼저 띄울 것")
            if waited == 50:
                print("[view] 파이프라인 대기 중 … (수신/추론이 뜨면 자동으로 붙는다)",
                      flush=True)
            time.sleep(0.1)
    ...
    def draw(res, base):
        ...
        for j, d in enumerate(sorted(res["dets"], key=lambda x: x["object"])):
            ...
            src = d.get("pose_source")
            if src == "soft_medoid":
                lb += " [soft]"
            elif src == "slam_anchor":
                lb += " [anchor]"
    ...
    try:
        while True:
            ...
            if got is not None or r is not None:
                last_data = time.time()
            elif time.time() - last_data > 3.0:
                # 파이프라인을 재시작하면 수신자가 공유메모리를 지우고 새로 만든다.
                # 기존 매핑은 영영 조용해지므로, 이름으로 다시 붙어 새 세그먼트를 잡는다.
                # (파이프라인이 그냥 쉬는 중이면 같은 세그먼트에 다시 붙을 뿐 — 무해)
                try:
                    nfr, njr = FrameReader(), JsonReader()
                    fr.close(); jr.close()
                    fr, jr = nfr, njr
                except FileNotFoundError:
                    pass                       # 아직 아무도 안 떠 있음 — 다음에 재시도
                last_data = time.time()
            if live is None:
                time.sleep(0.005)
                continue
            ...
            if web is not None:
                web.update(canvas, {
                    "hz_in": round(hz, 2),
                    "ms": (last[0].get("ms", {}) if last is not None else {}),
                    "n_proc": (last[0].get("n_proc", 0) if last is not None else 0),
                    "dets": ([{k: d.get(k) for k in
                               ("object", "score", "pose_source", "anchor_state", "drot")}
                              for d in last[0]["dets"]] if last is not None else []),
                })
            if not a.headless:
